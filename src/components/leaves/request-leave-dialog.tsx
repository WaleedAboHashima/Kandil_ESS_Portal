...
export function RequestLeaveDialog({
  ...
  const handleLeaveTypeChange = (leaveTypeId: string) => {
    const leaveType = leaveTypesData?.LeavesType.find(
      (lt) => lt.leave_type_id === Number(leaveTypeId)
    );
  ...
  // Hide trigger button when on dashboard route
  const showTrigger = location.pathname !== "/";

  return (
    <Dialog open={open} onOpenChange={setOpen}>
      {showTrigger && !preselectedDate && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Disabled dates are already requested / Date must be greater than
              today
            </p>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button
              type="submit"
              disabled={createLeave.isPending || !selectedLeaveType}
            >
